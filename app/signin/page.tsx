...
import { sendOTP, verifyLoginOTP } from "../redux/slices/authSlice";
...
function SignIn() {
  ...
  const handleContinue = async () => {
    ...
    if (cleanNumber.length === 10) {
      ...
      const type = "login";
      try {
        const res = await dispatch(sendOTP({ mobile, type })).unwrap();
        if (res?.success) {
          setIsOtpSent(true);
          setTimer(30);
        }
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await dispatch(sendOTP({ mobile, type: "login" })).unwrap();
      if (res?.success) {
        setOtp(["", "", "", ""]);
        setTimer(30);
      }
    ...
    if (otpValue.length === 4) {
      try {
        ...
        const res = await dispatch(
          verifyLoginOTP({ mobile: phone_number, otp: otpValue })
        ).unwrap();
        if (res?.success) {
